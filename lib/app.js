...
function ExpressApp(config) {
  ...
  app.post('/leads', publicAuth(config), validate({ body: LeadSchema }), (req, res) => {
    ...
    service.createLead(res.locals.accountId, res.locals.projectId, {
      system,
      tracking,
      lead
    })
  ...
  app.delete('/projects/:projectId/leads/:leadId', privateAuth(config), firebaseAuth(config), (req, res) => {
    service.deleteLead(res.locals.accountId, req.params.projectId, req.params.leadId)
    .then(({ system }) => {
      if (res.locals.timing) {
        res.set('x-capturoo-firebase-timing', system.firestoreMs);
      }
  ...
  app.get('/projects/:projectId', privateAuth(config), firebaseAuth(config), (req, res) => {
    service.getProject(res.locals.accountId, req.params.projectId)
      .then(({ system, project }) => {
        if (res.locals.timing) {
          res.set('x-capturoo-firebase-timing', system.firestoreMs);
        }
  ...
  app.post('/projects', privateAuth(config),
    firebaseAuth(config), validate({ body: NewProjectSchema }), (req, res) => {
      return service.createProject(res.locals.accountId, req.body.projectId, req.body.projectName)
      .then(({ system, project }) => {
        if (res.locals.timing) {
          res.set('x-capturoo-firebase-timing', system.firestoreMs);
        }
  ...
  app.delete('/projects/:projectId', privateAuth(config), firebaseAuth(config), (req, res) => {
    service.deleteProject(res.locals.accountId, req.params.projectId)
      .then(() => {
        res.status(204).end();
      })
      .catch(err => {
  ...
  app.get('/projects/:projectId/leads/:leadId', privateAuth(config), firebaseAuth(config), (req, res) => {
    service.getLead(res.locals.accountId, req.params.projectId, req.params.leadId)
      .then(({ system, lead }) => {
        if (res.locals.timing) {
          res.set('x-capturoo-firebase-timing', system.firestoreMs);
        }
  ...
  app.get('/projects/:projectId/leads', privateAuth(config), firebaseAuth(config), (req, res) => {
    ...
    service.getLeads(res.locals.accountId, req.params.projectId, options)
      .then(({ system, leads }) => {
        if (res.locals.timing) {
          res.set('x-capturoo-firebase-timing', system.firestoreMs);
        }
