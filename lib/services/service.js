...
/**
 * Publish a message on Google Pub/Sub service
 *
 * @param {object} payload
 * @param {object} attribs key value pairs of message attributes
 * @return {Promise.<number>}
 */
Service.prototype.publishMessage = async function(eventName, payload) {
  const pubsub = new PubSub({
    projectId: this.projectId,
    keyFilename: path.resolve(this.keyFilename)
  });

  try {
    return await pubsub.topic(this.topic)
      .publisher()
 ...
 */
Service.prototype.signUp = async (email, password, displayName) => {
  try {
    let createRequest = {
      disabled: false,
      displayName,
    ...
    let userRecord = await admin.auth().createUser(createRequest);
    let messageId = await this.publishMessage('CreateAccount', {
      userRecord
    });
 ...
 */
Service.prototype.createProject = async function(aid, pid, projectName) {
  try {
    const ts = process.hrtime();
    let docRef = admin.firestore().doc(`accounts/${aid}/projects/${pid}`);
    let docSnap = await docRef.get();
    if (docSnap.exists) {
      throw Error('projects/project-id-taken');
    }
    ...
    let messageId = await this.publishMessage('CreateProject', {
      ...result
    });
    console.log(`Message ${messageId} published.`);
    return result;
...
/**
 * GetProject: Get a project from an account by project ID
 ...
 * @returns {Promise.<ProjectInfo>}
 */
Service.prototype.getProject = async (aid, pid) => {
  try {
    const ts = process.hrtime();
    let docSnap = await admin.firestore().doc(`accounts/${aid}/projects/${pid}`).get();
    if (!docSnap.exists) {
...
Service.prototype.getProjects = async aid => {
  try {
    const ts = process.hrtime();
    const snapshot = await admin.firestore().collection(`accounts/${aid}/projects`).get();
    const projects = [];
 ...
 */
Service.prototype.deleteProject = async function(aid, pid) {
  try {
    let docRef = await admin.firestore().doc(`accounts/${aid}/projects/${pid}`);
    await this.deleteLeads(aid, pid);
    let docSnap = await docRef.get();
        if (!docSnap.exists) {
      throw Error('projects/project-not-found');
    }
 ...
 */
Service.prototype.getLead = async (aid, pid, lid) => {
  try {
    const ts = process.hrtime();
    let docSnap = await admin.firestore().collection(`accounts/${aid}/projects/${pid}/leads`).doc(lid).get();
 ...
 * @param {number} [options.startAfter] results start after the provided document (exclusive)
...
Service.prototype.getLeads = function getLeads(aid, pid, options) {
  return new Promise(function(resolve, reject) {
    ...
    if (opts.startAfter && typeof opts.startAfter !== 'number') {
      reject(TypeError('options.startAfter should be a number type'));
    }
    ...
    let orderBy = (opts.orderBy) ? opts.orderBy : 'system.created';
    let orderDirection = (opts.orderDirection) ? opts.orderDirection : 'desc';
      ...
      querySnapshot.forEach(doc => {
        ...
        Object.assign(l, {
          system: Object.assign(l.system, {
            created: l.system.created.toDate().getTime(),
            lid: doc.id
          })
        });
        leads.push(l);
